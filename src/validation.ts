...
export const personaNameSchema = z
  ...
  .regex(
    /^[a-zA-Z0-9_-]+$/,
    'Persona name can only contain letters, numbers, hyphens, and underscores'
  );
...
// 검증 헬퍼 함수
export function validatePersonaName(name: unknown): string {
  return personaNameSchema.parse(name);
}

export function validatePersonaContent(content: unknown): string {
